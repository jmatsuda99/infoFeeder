...
def add_feed(name,url,category=""):
    ...
    cur.execute("""INSERT INTO feeds(name,url,category,is_active,created_at,updated_at)
...
def update_feed_status(feed_id,is_active):
    conn=get_conn();cur=conn.cursor()
    cur.execute("UPDATE feeds SET is_active=? WHERE id=?",(1 if is_active else 0,feed_id))
    conn.commit();conn.close()
